...
fn line_matches(current_line: &String, pattern_to_find: &str) -> bool {
    let search = Regex::new(pattern_to_find).unwrap();
    search.is_match(current_line)
}
...
fn find_matches_generic_compile_time_style<W: Write>(
    ...
    for result_line in reader.lines() {
        let s = result_line?;
        if line_matches(&s, cli.pattern.as_str()) {
            writeln!(writer, "{}", s)?;
        }
    }
    Ok(())
